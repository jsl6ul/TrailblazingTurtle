...
def power(job, step):
    ...
    if job.gpu_count() > 0:
        ...
        query = '(label_replace(sum(redfish_chassis_power_average_consumed_watts{{{hostname_label}=~"({nodes}){oob_suffix}", {filter} }}) by ({hostname_label}), "{hostname_label}", "$1", "{hostname_label}", "(.*){oob_suffix}") \
